    ...
    <div style={{ background: C.offWhite, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "32px 20px" }}>
        ...
        <textarea
          ...
          placeholder="代官山の新しいラーメン屋。元アフリのシェフ。柚子塩が完璧。"
          rows={4}
          style={{ ...inputStyle, resize: "vertical" as const, marginBottom: 20 }}
        />

        {/* Google Maps URL */}
        <label style={{ fontFamily: F.ui, fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" as const, color: C.indigo, marginBottom: 8, display: "block" }}>
          📍 Google Maps URL（任意）
        </label>
        <input
          type="url" value={googleMapsUrl} onChange={e => setGoogleMapsUrl(e.target.value)}
        ...
        <input
          type="url" value={tabelogUrl} onChange={e => setTabelogUrl(e.target.value)}
          placeholder="https://tabelog.com/..."
          style={{ ...inputStyle, marginBottom: 20 }}
        />

        {/* Photos */}
        <label style={{ fontFamily: F.ui, fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" as const, color: C.indigo, marginBottom: 8, display: "block" }}>📷 Photos / 写真</label>
        <input ref={fileRef} type="file" accept="image/*" multiple onChange={e => addImages(e.target.files)} style={{ display: "none" }} />
        <button onClick={() => fileRef.current?.click()} style={{
          width: "100%", padding: "16px", border: `2px dashed ${C.lightWarm}`, borderRadius: 4,
          background: "transparent", fontFamily: F.ui, fontSize: 14, color: C.warmGray, cursor: "pointer", marginBottom: 12,
        }}>
          + タップして写真を追加
        </button>
            ...
            {previews.map((p, i) => (
              <div key={i} style={{ position: "relative", width: 80, height: 80 }}>
                ...
                <button onClick={() => removeImage(i)} style={{
                  ...
                  background: C.charcoal, color: "#fff", border: "none", fontSize: 11, cursor: "pointer", lineHeight: "20px",
                }}>×</button>
              </div>
            ))}
          </div>
        ...
        {/* Submit */}
        <button onClick={submit} disabled={status === "sending" || (!memo && images.length === 0)} style={{
          width: "100%", padding: "14px", background: status === "sending" ? C.warmGray : C.indigo, color: "#fff",
          border: "none", borderRadius: 4, fontFamily: F.ui, fontSize: 15, fontWeight: 600, letterSpacing: "0.06em",
          cursor: status === "sending" ? "wait" : "pointer", marginBottom: 20, opacity: (!memo && images.length === 0) ? 0.5 : 1,
        }}>
        ...
        {/* Result */}
        {status === "done" && (
          <div style={{ padding: 20, background: "#e8f5e9", borderRadius: 4, fontFamily: F.ui, fontSize: 14, color: "#2e7d32", whiteSpace: "pre-wrap" }}>
            ✅ 記事が作成されました！\n\n{result}
          </div>
        )}
        {status === "error" && (
          <div style={{ padding: 20, background: "#fce4ec", borderRadius: 4, fontFamily: F.ui, fontSize: 14, color: "#c62828" }}>
            ❌ エラー: {result}
          </div>
        )}
        ...
        <div style={{ marginTop: 40, padding: 24, background: C.cream, borderRadius: 4 }}>
          ...
          <div style={{ fontFamily: F.body, fontSize: 14, lineHeight: 1.8, color: C.charcoal }}>
            ...
            <p style={{ marginBottom: 8 }}>4. 写真を追加（複数可）</p>
            <p style={{ marginBottom: 0 }}>5.「記事を生成する」→ AIが英語記事を自動作成 → Sanityに下書き保存</p>
          </div>
        </div>

        {/* LINE Bot guide */}
        <div style={{ marginTop: 16, padding: 24, background: C.cream, borderRadius: 4 }}>
          <div style={{ fontFamily: F.ui, fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" as const, color: C.indigo, marginBottom: 12 }}>LINE Botでも投稿可能</div>
          <div style={{ fontFamily: F.body, fontSize: 14, lineHeight: 1.8, color: C.charcoal }}>
            <p style={{ marginBottom: 8 }}>TONE TOKYO Botに直接メッセージを送信：</p>
            <p style={{ marginBottom: 8, fontFamily: F.ui, fontSize: 13, color: C.warmGray, fontStyle: "italic" }}>
              「代官山の新しいラーメン屋。柚子塩が完璧。」<br/>
              https://maps.google.com/...<br/>
              https://tabelog.com/...
            </p>
            <p>URLはメモと一緒にそのまま貼付すればOK</p>
          </div>
        </div>
      </div>
    </div>
