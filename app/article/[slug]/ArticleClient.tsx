...
export default function ArticleClient({ article, related }: { article: any; related: any[] }) {
  ...
  const A = article;
  const heroImg = A.heroImage || FALLBACK;
  ...
  return (<><ProgressBar /><StickyHeader visible={stickyVisible} pillar={A.pillar} readTime={A.readTime} /><div style={{ background: C.offWhite, minHeight: "100vh" }}>
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 24px" }}>
      ...
      {related.length > 0 && <div style={{ padding: "48px 0 0" }}><div style={{ fontFamily: F.ui, fontSize: 11, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase" as const, color: C.warmGray, marginBottom: 24 }}>READ NEXT</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 28 }}>{related.map((r: any, i: number) => <Link key={i} href={"/article/" + r.slug} style={{ textDecoration: "none", color: "inherit" }}><div style={{ aspectRatio: "4/3", overflow: "hidden", background: C.cream, marginBottom: 14 }}><img src={r.heroImage || FALLBACK} alt={r.title} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div><div style={{ fontFamily: F.ui, fontSize: 10, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase" as const, color: pillarColor(r.pillar), marginBottom: 8 }}>{r.pillar}</div><div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 700, lineHeight: 1.35, color: C.charcoal, marginBottom: 6 }}>{r.title}</div><div style={{ fontFamily: F.ui, fontSize: 11, color: C.warmGray }}>{r.readTime}</div></Link>)}</div></div>}
