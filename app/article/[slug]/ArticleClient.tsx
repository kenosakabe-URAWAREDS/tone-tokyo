...
import { sizedImage, urlForArticleImage, urlForSanityImage, objectPositionFromHotspot } from "@/lib/image";
...
function Gallery({ items }: { items: any[] }) {
  ...
  // 3-column grid, 4:3 aspect, placed ABOVE Details per spec B3.
  return (
    <div style={{ padding: "48px 0 0" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {items.map((item, i) => {
          const src = urlForSanityImage(item, { w: 800, h: 600, q: 80 });
          if (!src) return null;
...
function usePVTracker(slug?: string, pillar?: string) {
  useEffect(() => {
    if (!slug) return;
    fetch('/api/track-view', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ slug, pillar }),
    }).catch(() => {});
  }, [slug, pillar]);
}
...
export default function ArticleClient({ article, related }: { article: any; related: any[] }) {
  ...
  usePVTracker(article?.slug?.current || article?.slug, article?.pillar);
  const A = article;
  ...
  // 2400x1350 — same approach as HomeClient. urlForArticleImage falls
  ...
  return (<><ProgressBar /><StickyHeader visible={stickyVisible} pillar={A.pillar} readTime={A.readTime} /><div style={{ background: C.offWhite, minHeight: "100vh" }}>
    ...
    <div ref={heroRef} style={{ position: "relative", width: "100%", aspectRatio: "16/9", minHeight: 420, maxHeight: 500, overflow: "hidden" }}><img src={heroImg} alt={A.title} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: heroPos }} /><div style={{ position: "absolute", bottom: 0, left: 0, width: "100%", background: "linear-gradient(transparent, rgba(0,0,0,0.55))", height: "50%" }} />{A.heroCaption && <div style={{ position: "absolute", bottom: 16, right: 24, fontFamily: F.ui, fontSize: 11, color: "rgba(255,255,255,0.65)", fontStyle: "italic" }}>{A.heroCaption}</div>}</div>
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 24px" }}>
      ...
      {related.length > 0 && <div style={{ padding: "48px 0 0" }}><div style={{ fontFamily: F.ui, fontSize: 11, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase" as const, color: C.warmGray, marginBottom: 24 }}>READ NEXT</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 28 }}>{related.map((r: any, i: number) => <Link key={i} href={"/article/" + r.slug} style={{ textDecoration: "none", color: "inherit" }}><div style={{ aspectRatio: "3/2", overflow: "hidden", background: C.cream, marginBottom: 14 }}><img src={urlForArticleImage(r, { w: 600, h: 400 }) || sizedImage(r.heroImage || FALLBACK, 600)} alt={r.title} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: objectPositionFromHotspot(r.heroImageHotspot) }} /></div><div style={{ fontFamily: F.ui, fontSize: 10, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase" as const, color: pillarColor(r.pillar), marginBottom: 8 }}>{r.pillar}</div><div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 700, lineHeight: 1.35, color: C.charcoal, marginBottom: 6 }}>{r.title}</div><div style={{ fontFamily: F.ui, fontSize: 11, color: C.warmGray }}>{r.readTime}</div></Link>)}</div></div>}
